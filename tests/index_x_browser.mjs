...
try{await page.goto('https://corbanu.com/indexes/?index=fixture');await page.getByRole('heading',{name:'X claim test',exact:true}).waitFor();await page.locator('#creator-tools > summary').click();assert.equal(await page.locator('#claim-index-x').isEnabled(),true);assert.equal(await page.locator('#claim-index').isDisabled(),true);await page.locator('#claim-index-x').click();await page.waitForURL('https://x.com/**');assert.equal(starts,1);assert.deepEqual(errors,[]);console.log('X claim starts without a MetaMask wallet and navigates to authorization.');}finally{await browser.close();}
